...
function App() {
  ...
  const updateOrderStatus = async (id, status) => {
    ...
    const response = await fetch(`/api/v1/orders/${id}/order/`, options).catch(
      handleError
    );
  ...
  const addToOrder = (item) => {
    ...
    if (!order.find((i) => i.name === orderItem.name)) {
      ...
      order[index].price =
        parseFloat(order[index].price) + parseFloat(orderItem.price);
    }
  };
  ...
  const checkOrder =
    totalAmount === 0 ? (
      ...
      <span className="order-num">{totalAmount}</span>
    );
    ...
    <div className="App">
      <header>
        <nav className="navbar">
          <button className="homepage" onClick={() => setScreen("homescreen")}>
            <FontAwesomeIcon
              icon={faFish}
              style={{ color: "#B55A2B", fontSize: "3rem", marginLeft: "1rem" }}
            />
          </button>
          <ul className="nav-list">
            <li className="nav-item">
              <button onClick={() => setScreen("menuScreen")}>Menu</button>
            </li>
            <li className="nav-item">
              <button onClick={() => setScreen("orderScreen")}>Order</button>
              {checkOrder}
            </li>
      ...
      {screen === "menuScreen" && (
        <MenuList items={items} addToOrder={addToOrder} formatter={formatter} />
      )}
      {screen === "orderScreen" && (
        <Order
